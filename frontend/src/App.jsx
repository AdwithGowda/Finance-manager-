...
function App() {
  ...
  // Updated form state with default today's date
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Reset form to default including today's date
      setForm({ 
        title: '', 
        amount: '', 
        category: 'Food', 
  ...
  return (
    <div className="min-h-screen bg-[#f8fafc] text-slate-900 font-sans antialiased selection:bg-indigo-100 selection:text-indigo-900">
      <div className="max-w-6xl mx-auto px-4 py-8 md:px-6 lg:py-12 space-y-10">
        ...
        <header className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-6 bg-indigo-600 p-8 rounded-3xl shadow-lg border-0">
          <div>
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center w-full sm:w-auto gap-4 sm:gap-6">
            <div className="flex-1 sm:flex-none text-right">
              <p className="text-xs uppercase font-bold tracking-widest text-indigo-200 mb-2">Main Balance</p>
            ...
            <div className="flex-1 sm:flex-none text-right">
              <p className="text-xs uppercase font-bold tracking-widest text-indigo-200 mb-2">Other Balance</p>
              <p className="text-3xl font-black text-white tabular-nums">
            ...
            <section className={`bg-white p-6 rounded-3xl shadow-sm border transition-all duration-300 ${editingId ? 'border-orange-500 shadow-lg shadow-orange-100' : 'border-slate-200/60'}`}>
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                {editingId && (
                  <button 
                    type="button"
                    onClick={() => {setEditingId(null); setForm({title:'', amount:'', category:'Food', date: new Date().toISOString().split('T')[0]}); setIsOtherSpend(false);}}
          ...
          <div className="lg:col-span-8 space-y-6">
            <div className="space-y-4">
              ...
              <div className="flex gap-3">
                <button onClick={() => setFilterMode('main')} className={`flex-1 py-3 px-4 rounded-2xl font-semibold text-sm transition-all duration-200 border-2 ${filterMode === 'main' ? 'bg-indigo-600 text-white border-indigo-600 shadow-lg shadow-indigo-200' : 'bg-white text-slate-700 border-slate-200 hover:border-indigo-300'}`}>
                  Main
                </button>
                <button onClick={() => setFilterMode('other')} className={`flex-1 py-3 px-4 rounded-2xl font-semibold text-sm transition-all duration-200 border-2 ${filterMode === 'other' ? 'bg-indigo-600 text-white border-indigo-600 shadow-lg shadow-indigo-200' : 'bg-white text-slate-700 border-slate-200 hover:border-indigo-300'}`}>
                  Other Spend
                </button>
              </div>
            </div>
                      ...
                      <tr key={exp.id} className="hover:bg-indigo-50/30 transition-colors group">
                        ...
                        <td className="px-6 py-5 text-center">
                          <div className="flex justify-center gap-3">
                            <button onClick={() => handleEdit(exp)} className="p-2 text-indigo-500 hover:bg-indigo-100 rounded-lg transition-colors"><svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" /></svg></button>
                            <button onClick={() => handleDeleteTrigger(exp.id)} className="p-2 text-red-400 hover:bg-red-50 hover:text-red-600 rounded-lg transition-colors"><svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" /></svg></button>
                          </div>
                        </td>
                      </tr>
                    ))}

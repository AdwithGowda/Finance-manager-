...
export default function Auth({ onAuthSuccess }) {
  ...
  const submit = async (e) => {
    ...
    try {
      ...
      // Included username in the payload
      ...
      if (isLogin) {
        ...
        setEmail("");
        setPassword("");
        setUsername(""); // Clear username after register
      }
    } catch (err) {
      setIsLoading(false);
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-[#f8fafc] text-slate-900 font-sans antialiased">
      <form
        onSubmit={submit}
        className="bg-white p-8 rounded-3xl shadow-lg border border-slate-200/60 w-full max-w-md space-y-5"
      >
        ...
        {/* USERNAME FIELD - Only shows during Registration */}
        {!isLogin && (
